@php
    $html_tag_data = [];
    $title = 'Data Bobot KPI';
    $description = 'View Bobot KPI';
    ...
    <div class="container">
        ...
        <div class="card mb-5">
            <div class="card-body table-responsive">
                ...
                <table class="table table-sm">
                    <thead>
                        <tr>
                            <th scope="col">#</th>
                            <th scope="col" class="text-nowrap">Indikator</th>
                            <th scope="col" class="text-nowrap text-end">Target</th>
                            <th scope="col" class="text-nowrap text-end">Bobot</th>
                            <th scope="col" class="text-nowrap text-end">Terlambat</th>
                            <th scope="col" class="text-nowrap text-center">-</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if (count($indicatorWeights) == 0)
                            <tr>
                                <td colspan="8" class="text-center text-muted">Tidak ada data</td>
                            </tr>
                        @else
                            @foreach ($indicatorWeights as $data)
                                <tr>
                                    <td class="align-middle">{{ $loop->iteration }}</td>
                                    <td class="align-middle">{{ $data->type }}</td>
                                    <td class="align-middle text-end">{{ number_format($data->target) }}</td>
                                    <td class="align-middle text-end">{{ number_format($data->weight_percentage) }}%</td>
            ...
            <div class="card-footer pt-2 pb-2">
                {{ $indicatorWeights->appends(['query' => $query])->links() }}
            </div>
        </div>
        <!-- Item List End -->
    </div>
